...
void GenerateTextures()
{
	...
	GLboolean error = load_targa("neptun.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku neptun.tga");
		exit(0);
	}
	...
	error = load_targa("neptun.tga", width, height, format, type, pixels);
